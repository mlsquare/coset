#!/usr/bin/env python3
"""
Decoding Performance Profiler

This script profiles the decoding process for hierarchical nested-lattice quantization,
comparing baseline PyTorch implementation against CUDA-optimized versions.

Usage:
    python benchmarks/profile_decoding.py [--lattice D4] [--q 4] [--M 2] [--batch-sizes 100,1000,10000]
"""
...
from coset.cuda.kernels import cuda_decode, is_cuda_available


class DecodingProfiler:
    """Profiler for decoding operations with baseline and CUDA implementations."""
    ...
    def generate_test_encodings(self, batch_size: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        """Generate test encodings for profiling."""
        # Generate random vectors and encode them
        d = self.lattice.d
        x = torch.randn(batch_size, d, device=device, dtype=torch.float32) * 0.5
        ...
        encodings = torch.stack(encodings)  # [batch_size, M, d]
        t_values = torch.tensor(t_values, device=device, dtype=torch.int32)
        
        return encodings, t_values
    
    def baseline_decode(self, encodings: torch.Tensor, t_values: torch.Tensor) -> torch.Tensor:
        """
        Baseline decoding using current PyTorch implementation.
        
        Args:
            encodings: Encoded vectors of shape [batch_size, M, d]
            t_values: Scaling counts of shape [batch_size]
            
        Returns:
            Decoded vectors of shape [batch_size, d]
        """
        batch_size = encodings.shape[0]
        decoded = []
        
        for i in range(batch_size):
            x_hat = decode(encodings[i], self.lattice, self.config, t_values[i].item())
            decoded.append(x_hat)
        
        return torch.stack(decoded)
    
    def cuda_optimized_decode(self, encodings: torch.Tensor, t_values: torch.Tensor) -> torch.Tensor:
        """
        CUDA-optimized decoding using actual CUDA kernels.
        ...
        if not self.cuda_available:
            ...
            return self.baseline_decode(encodings, t_values)
        
        try:
            # Use real CUDA kernel
            return cuda_decode(encodings, t_values, self.lattice, self.config)
        except Exception as e:
            print(f"CUDA decoding failed, falling back to baseline: {e}")
            return self.baseline_decode(encodings, t_values)
    
    def profile_single_batch(self, batch_size: int, num_warmup: int = 5, num_iterations: int = 20) -> Dict[str, Any]:
        """
        Profile decoding for a single batch size.
        ...
        encodings, t_values = self.generate_test_encodings(batch_size, self.device)
        ...
        for _ in range(num_warmup):
            _ = self.baseline_decode(encodings, t_values)
            if self.cuda_available:
        ...
        for _ in range(num_iterations):
            ...
            decoded = self.baseline_decode(encodings, t_values)
        ...
        # Warmup
        for _ in range(num_warmup):
            _ = self.cuda_optimized_decode(encodings, t_values)
            if self.cuda_available:
                torch.cuda.synchronize()
        ...
        for _ in range(num_iterations):
            ...
            start_time = time.perf_counter()
            decoded = self.cuda_optimized_decode(encodings, t_values)
    ...
    def profile_multiple_batches(self, batch_sizes: List[int]) -> List[Dict[str, Any]]:
        """
        Profile decoding for multiple batch sizes.
    ...
    def plot_results(self, results: List[Dict[str, Any]], output_dir: Path):
        ...
        ax1.set_title(f'Decoding Time vs Batch Size\n({self.lattice_type}, q={self.q}, M={self.M})')
        ax1.legend()
        ...
        ax3.set_title('Decoding Throughput vs Batch Size')
        ax3.legend()
        ax3.grid(True, alpha=0.3)
        
        # Plot 4: Memory bandwidth efficiency
        # Calculate memory bandwidth: data_size / time
        ...
        baseline_bandwidth = [size / (r['baseline']['mean_time'] * 1e9) for size, r in zip(encoded_size, results)]  # GB/s
        cuda_bandwidth = [size / (r['cuda_optimized']['mean_time'] * 1e9) for size, r in zip(encoded_size, results)]  # GB/s
        
        ax4.semilogx(batch_sizes, baseline_bandwidth, 'o-', label='Baseline (PyTorch)', linewidth=2, markersize=8)
        ax4.semilogx(batch_sizes, cuda_bandwidth, 's-', label='CUDA Optimized', linewidth=2, markersize=8)
        ax4.set_xlabel('Batch Size')
        ax4.set_ylabel('Memory Bandwidth (GB/s)')
        ax4.set_title('Memory Bandwidth vs Batch Size')
        ax4.legend()
        ...
        # Save the plot
        plot_path = output_dir / f'decoding_profile_{self.lattice_type}_q{self.q}_M{self.M}.png'
    ...
    def save_results(self, results: List[Dict[str, Any]], output_dir: Path):
        ...
        import json
        results_path = output_dir / f'decoding_results_{self.lattice_type}_q{self.q}_M{self.M}.json'
        ...
        # Save summary as CSV
        import csv
        csv_path = output_dir / f'decoding_summary_{self.lattice_type}_q{self.q}_M{self.M}.csv'
...
def main():
    """Main function for the decoding profiler."""
    parser = argparse.ArgumentParser(description='Profile decoding performance')
    ...
    print(f"Decoding Performance Profiler")
    ...
    # Initialize profiler
    profiler = DecodingProfiler(lattice_type=args.lattice, q=args.q, M=args.M)
