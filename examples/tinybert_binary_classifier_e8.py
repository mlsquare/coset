#!/usr/bin/env python3
"""
Pre-trained TinyBERT Binary Classifier with Quantized Head Example

This example demonstrates how to use a pre-trained TinyBERT model with a quantized
binary classification head using the new constructor-based QAT layers:
- Load pre-trained TinyBERT from transformers library
...
- Keep TinyBERT frozen during training
"""
...
class QuantizedTinyBERTBinaryClassifier(nn.Module):
    """TinyBERT binary classifier with quantized classification head."""
    
    def __init__(self, model_name='prajjwal1/bert-tiny', warmup_epochs=2, 
                 weight_clip_value=2.0, freeze_tinybert=True):
        super().__init__()
        
        # Load pre-trained TinyBERT
        print(f"Loading pre-trained TinyBERT model: {model_name}")
        self.tinybert = AutoModel.from_pretrained(model_name)
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        
        # Freeze TinyBERT parameters if requested
        if freeze_tinybert:
            for param in self.tinybert.parameters():
                param.requires_grad = False
            print("✓ TinyBERT parameters frozen")
        else:
            print("✓ TinyBERT parameters trainable")
        
        # Get TinyBERT hidden size
        tinybert_hidden_size = self.tinybert.config.hidden_size
        print(f"TinyBERT hidden size: {tinybert_hidden_size}")
        
        # Ensure hidden size is divisible by 8 for E8 lattice
        if tinybert_hidden_size % 8 != 0:
            # Add a projection layer to make it divisible by 8
            self.projection = nn.Linear(tinybert_hidden_size, ((tinybert_hidden_size + 7) // 8) * 8)
            classifier_input_size = ((tinybert_hidden_size + 7) // 8) * 8
            print(f"Added projection layer: {tinybert_hidden_size} -> {classifier_input_size}")
        else:
            self.projection = None
            classifier_input_size = tinybert_hidden_size
    ...
    def forward(self, input_ids, attention_mask=None):
        # Get TinyBERT outputs
        outputs = self.tinybert(input_ids=input_ids, attention_mask=attention_mask)
        
        # Use pooler_output if available, otherwise use CLS token embedding
        if hasattr(outputs, 'pooler_output') and outputs.pooler_output is not None:
            pooled_output = outputs.pooler_output  # [batch_size, hidden_size]
        else:
            # Fallback: use CLS token (first token) embedding
...
def create_synthetic_binary_data(num_samples=500, max_length=128):
...
def load_data(model_name='prajjwal1/bert-tiny', data_fraction=0.8, batch_size=16):
...
def train_model(model, train_loader, test_loader, epochs=10, lr=0.001):
    """Train the TinyBERT binary classifier with QAT on classification head only."""
    ...
    print(f"\nTraining TinyBERT binary classifier with QAT for {epochs} epochs...")
    print("Note: Only the classification head is being trained (TinyBERT is frozen)")
...
def main():
    """Main function to run the TinyBERT binary classifier example."""
    print("Pre-trained TinyBERT Binary Classifier with Quantized Head Example")
    print("=" * 70)
    ...
    freeze_tinybert = True  # Only train the classification head
    
    # Load data
    print("Loading and tokenizing synthetic binary text data...")
    train_loader, test_loader = load_data(model_name=model_name, data_fraction=0.1, batch_size=16)
    print(f"Training samples: {len(train_loader.dataset)}")
    print(f"Test samples: {len(test_loader.dataset)}")
    
    # Create model
    print(f"\nCreating TinyBERT binary classifier with quantized head...")
    model = QuantizedTinyBERTBinaryClassifier(
        model_name=model_name,
        warmup_epochs=warmup_epochs,
        weight_clip_value=weight_clip_value,
        freeze_tinybert=freeze_tinybert
    )
    ...
    # Train model
    train_model(model, train_loader, test_loader, epochs=1, lr=0.001)
    
    print("\nTraining completed!")
    print("This example demonstrates:")
    print("- Pre-trained TinyBERT with frozen parameters")
